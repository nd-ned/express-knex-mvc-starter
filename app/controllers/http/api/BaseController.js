...
class BaseController {
    ...
    index = async (req, res, next) => {
        try {
            return res.apiOK("Entities successfully retrieved", await this.Model.findAll({}))
        } catch (e) {
            next(e)
        }
    }
    ...
    create = async (req, res, next) => {
        try {
            const createColumns = this.Model.columns.filter(c => !['id', 'created_at', 'updated_at'].includes(c))
            const createData = {}
    ...
    update = async (req, res, next) => {
        try {
            if (parseInt(req.params.id) || !await this.Model.findById(req.params.id)) {
                return res.apiNotFound("Entity not found!")
            }
            
            const updateColumns = this.Model.columns.filter(c => !['id', 'created_at', 'updated_at'].includes(c))
            const updateData = {}
            ...
            return res.apiOK("Entity successfully updated", await this.Model.findById(req.params.id))
        } catch (e) {
            next(e)
        }
    }
